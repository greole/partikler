#ifndef SEARCHCUBE3_H
#define SEARCHCUBE3_H
...
struct UnsortedSearchCubes3 {

    std::vector<bool> empty;

    std::vector<SearchCube3> search_cubes;
};

class SearchCubes3: public SearchCubesBase<SearchCube3> {
    ...
    //     std::vector<SearchCube3>& searchCubes,
    ...
    //         ) {
    //         UnsortedSearchCubes3 tmp_searchCubes =
    ...
    LeanParticleNeighbours compute_particle_neighbours(
        ...
        for (size_t sid=0; sid < tot_n_cubes_; sid++) {
    ...
    // UnsortedSearchCubes3 construct_particle_search_cube_relation(
    //         const std::vector<Point>& points,
    //         std::vector<std::vector<size_t>>& pidu_kid
    //         )
    //     {
    ...
    const std::vector<SearchCube3>& get_searchCubes() const {
        return searchCubes_;
    };
    ...
    void  set_particle_neighbours(
            ...
            const size_t first = first_[sid];
            const size_t last = last_[sid];

            if (first == n_points_) return;

            for (size_t pid = first; pid <= last; pid++) {
                ...
                for (size_t nid = first; nid <= last; nid++) {
                    ...
                    const Point& npoint {sorted_points[nid]};
