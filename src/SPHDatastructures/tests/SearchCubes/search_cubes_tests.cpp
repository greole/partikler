...
bool found_particle_pair(size_t i, size_t j, size_t k, SortedNeighbours & ret ) {
  bool found_i = (ret.ownId[k] == i);
  bool found_j = (ret.neighId[k] == j);
...
TEST (OwnerCubeSearch, FindsNeighbours) {
  ...
  ret.ownId.reserve(tot_n_particles*points.size()/2);
  ret.neighId.reserve(tot_n_particles*points.size()/2);

  owner_cube_search(points, 0, tot_n_particles, 5.0, ret);

  // Should find at least some neighbours
  ASSERT_GT(ret.ownId.size(), 0);

  // Neighbourhood should be found once at max
  ASSERT_LT(ret.ownId.size(), tot_n_particles*tot_n_particles);

  // When each particle has all particles as neighbour
  // the number of particle pairs is n-1+n-2+...
  ASSERT_EQ(ret.ownId.size(), (tot_n_particles-1)/2*tot_n_particles);

  // Every particle needs to be found either as owner or neighbour
  for (size_t i=0; i<tot_n_particles; i++) {
    bool present =
      std::find(ret.ownId.begin(), ret.ownId.end(), i) != ret.ownId.end()
      ||
      std::find(ret.neighId.begin(), ret.neighId.end(), i) != ret.neighId.end();
  ...
  for (size_t i=0; i<tot_n_particles; i++) {
    ...
    for (size_t j=i+1; j<tot_n_particles; j++) {
      for (size_t k = 0; k<ret.ownId.size(); k++){
...
TEST (NeighbourCubeSearch, FindsNeighbours) {
  ...
  ASSERT_EQ(ret.ownId.size(), tot_n_particles*tot_n_particles);
...
TEST (ParticleNeighbourSearch, createNeighbours) {
  ...
  ASSERT_GT(ret.ownId.size(), estimate_n_neighbours*3/4);
  ASSERT_LT(ret.ownId.size(), estimate_n_neighbours*4/3);
