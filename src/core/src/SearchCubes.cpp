...
initSearchCubeDomain(const std::vector<Point> &particles, float dx) {
    ...
    Logger(3, "initSearchCubeDomainSTL").check(n_cubes_x >= 3)
        << " n_cubes_x less than 3";
    Logger(3, "initSearchCubeDomainSTL").check(n_cubes_y >= 3)
...
size_t position_to_cube_id_vec3(SearchCubeDomain scd, const Vec3 &p) {
    ...
    const Scalar mx = scd.min.x;
    const Scalar my = scd.min.y;
    const Scalar mz = scd.min.z;
...
SortedParticles countingSortParticles(
    ...
    for (size_t i = 0; i < n_particles; i++) {
        count[position_to_cube_id_vec3(scd, unsorted_particles[i])]++;
    }
