...
void SPHParticleGenerator::execute() {
    ...
    // to Vec3. This is to encapsulate CGAL from the rest of
    // the Partikler sources. Where ever CGAL functionality is
    // needed Vec3 can be converted back to CGAL Points.
    // Here performance should not be too important.
    // TODO
    // Find a simple way for conversion or wrapping of CGAL points
    log().info_begin() << "Generating initial particles ";
    Generate_Points_at_Facets gpf(dx_, points, facets_);

    size_t n_0 = points.size();
